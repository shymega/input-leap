...
stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = [ pkg-config cmake wrapGAppsHook3 qt6.wrapQtAppsHook qt6.qttools ];
  buildInputs = [
    curl qt6.qtbase (avahi.override { withLibdnssdCompat = true; }) openssl
    libX11 libXext libXtst libXinerama libXrandr libXdmcp libICE libSM
    gtest gmock
] ++ lib.optionals (withLibei && !isDarwin) (with pkgs; [ libei libportal qt6.qtwayland ]);

  cmakeFlags = [
    "-DINPUTLEAP_REVISION=${version}"
